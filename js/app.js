...
async function renderDashboard() {
  ...
  const prefs = await Discover.getPreferenceWeights();
  ...
  for (const t of topics) {
    const cat = t.primaryCategory || t.category || 'Uncategorized';
    categoryCounts[cat] = (categoryCounts[cat] || 0) + 1;
  }
  const categoryBreakdown = Object.entries(categoryCounts).sort((a, b) => b[1] - a[1]).slice(0, 8);
  const topPreferredCategories = Object.entries(prefs.categories || {}).sort((a, b) => b[1] - a[1]).slice(0, 8);
  ...
  return `
    ...
    <div class="grid two-col">
      <div class="card">
        <h3>Library by category<span class="card-sub">primary topic category, not discovery mode</span></h3>
        ${categoryBreakdown.length ? `<ul class="list">${categoryBreakdown.map(([cat, n]) => `
          <li class="list-row"><span>${escapeHtml(cat)}</span><span class="muted small">${n}</span></li>`).join('')}</ul>`
          : emptyState('No categorized topics yet.')}
      </div>
      <div class="card">
        <h3>Your taste profile<span class="card-sub">learned from saves, rejections, publishes & deletes — never overrides the topic score itself</span></h3>
        ${topPreferredCategories.length ? `
          <p class="muted small">Categories you engage with most:</p>
          <ul class="list">${topPreferredCategories.map(([cat, w]) => `<li class="list-row"><span>${escapeHtml(cat)}</span><span class="muted small">${Math.round(w)}/100</span></li>`).join('')}</ul>`
          : emptyState('No preference signal yet.', 'Save, reject, or publish topics from Discover to build your taste profile.')}
      </div>
    </div>
  `;
}
      ...
      <div class="row-between wrap">
        ...
        <div class="row gap">
          <select id="discover-mode">
            ${Taxonomy.DISCOVERY_MODES.map((m) => `<option value="${m.id}">${escapeHtml(m.label)}</option>`).join('')}
          </select>
...
function renderIdeaCard(idea, i) {
  ...
  return `
    <div class="card idea-card" id="idea-card-${i}">
      <div class="row-between wrap">
        <span class="pill">${escapeHtml(idea.primaryCategory)}</span>
        <span class="pill pill-muted">${escapeHtml(idea.realityStatus)}</span>
      </div>
      <div class="row gap wrap">
        <span class="tag">${Icon('tag')}${escapeHtml(idea.subcategory || '—')}</span>
        <span class="tag">${Icon('tag')}Angle: ${escapeHtml(idea.angle)}</span>
        <span class="tag">${Icon('tag')}Mode: ${escapeHtml(idea.discoveryModeLabel)}</span>
      </div>
      ...
      ${forecastRow}
      <p class="muted small">Confidence: ${idea.confidence} · relevance to your taste: ${idea.relevance}/100 · reason: ${escapeHtml(idea.forecast.reason)}</p>
      <div class="row gap">
        <button class="btn btn-ghost grow reject-idea-btn" data-idx="${i}">${Icon('x')} Not interested</button>
  ...
  $('#generate-btn').addEventListener('click', async () => {
    ...
    $all('.save-idea-btn').forEach((b) => b.addEventListener('click', async () => {
      ...
      await DB.add('topics', {
        ...
        angle: idea.angle,
        discoveryMode: idea.discoveryMode,
        status: 'idea',
        ...
        discoveryMeta: { mode: idea.discoveryMode, realityStatus: idea.realityStatus, angle: idea.angle },
        createdAt: now,
        updatedAt: now,
      });
      await Discover.recordFeedback(idea, 'saved');
    ...
    $all('.reject-idea-btn').forEach((b) => b.addEventListener('click', async () => {
      const idx = Number(b.dataset.idx);
      const idea = lastIdeas[idx];
      await Discover.rejectIdea(idea);
      const card = $(`#idea-card-${idx}`);
      if (card) { card.style.opacity = '0.4'; card.style.pointerEvents = 'none'; }
      toast('Got it — you\u2019ll see fewer like this', 'default');
    }));
  });
  $('#generate-btn').click();
}
...
async function renderTopicAnalyzer() {
  const categories = Taxonomy.CATEGORY_NAMES;
  return `
    <div class="grid two-col">
      <div class="card">
        <h3>Enter a topic</h3>
...
function renderScriptResults(r) {
  const dimLabels = {
    ...
    narrativeCoherence: 'Narrative coherence', conceptClarity: 'Concept clarity',
    informationDensity: 'Information density', openLoopStrength: 'Open loop strength', endingStrength: 'Ending strength',
  };
  return `
    <div class="row-between">
      <h3>Script impact</h3>
    ...
    <h4>Narrative beat timeline</h4>
    <div class="timeline">
      ${r.beats.map((b) => `
        <div class="timeline-seg ${b.hookStrength < 40 ? 'risk' : ''}">
          <strong>${escapeHtml(b.function)}</strong>
          <span class="muted small">"${escapeHtml(b.text.length > 60 ? b.text.slice(0, 60) + '…' : b.text)}"</span>
          <span class="muted small">hook ${b.hookStrength} · visual ${b.visualOpportunity} · curiosity: ${b.curiosityStatus} · escalation: ${b.escalationStatus}</span>
        </div>`).join('')}
    </div>

    <div class="grid two-col">
      <div><h4>Strongest beat</h4><p class="muted small">${escapeHtml(r.strongestSection)}</p></div>
      <div><h4>Weakest beat</h4><p class="muted small">${escapeHtml(r.weakestSection)}</p></div>
    </div>
    <h4>Flags & suggestions</h4>
    <ul class="flag-list">
      ${r.factDumpSections.length ? `<li class="warn">${Icon('alert')} Possible fact-dump beat(s): ${r.factDumpSections.join(' | ')} — consider adding a question or a "but/except" turn.</li>` : `<li class="pos">${Icon('check')} No fact-dump beats detected.</li>`}
      ${r.predictable ? `<li class="warn">${Icon('alert')} Low hook/open-loop density and little emotional variation overall — the script may read as predictable.</li>` : `<li class="pos">${Icon('check')} Hook density and emotional variation look healthy.</li>`}
      ${!r.hasTransformation ? `<li class="warn">${Icon('alert')} No hypothetical transformation (explicit or implicit) detected — consider adding a clear speculative turn.</li>` : `<li class="pos">${Icon('check')} Hypothetical transformation detected (explicit and/or implicit).</li>`}
      ${!r.hasPayoff ? `<li class="warn">${Icon('alert')} No clear payoff near the end.</li>` : `<li class="pos">${Icon('check')} Payoff (final question/revelation) detected near the end.</li>`}
      <li class="${r.hasCTA ? 'pos' : 'neutral'}">${Icon(r.hasCTA ? 'check' : 'alert')} CTA: ${r.hasCTA ? 'Present' : 'Not present'}. ${escapeHtml(r.ctaRecommendation)}</li>
      <li class="muted small">${Icon('insights')} Emotional progression touched ${r.distinctEmotionCount} distinct state(s) across the script.</li>
    </ul>
  `;
}
...
function wireLibraryCards() {
  $all('.status-select').forEach((s) => s.addEventListener('change', async () => {
    ...
    const feedbackIdea = { primaryCategory: t.primaryCategory || t.category, angle: t.angle, discoveryMode: t.discoveryMode };
    if (s.value === 'published' && prevStatus !== 'published') await Discover.recordFeedback(feedbackIdea, 'published');
    else if (s.value === 'rejected' && prevStatus !== 'rejected') await Discover.recordFeedback(feedbackIdea, 'rejected');
  ...
  $all('.delete-topic-btn').forEach((b) => b.addEventListener('click', async () => {
    ...
    const t = await DB.get('topics', Number(b.dataset.id));
    if (t) await Discover.recordFeedback({ primaryCategory: t.primaryCategory || t.category, angle: t.angle, discoveryMode: t.discoveryMode }, 'deleted');
